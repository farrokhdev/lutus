...
class ConversationBox extends Component {
  ...
  _getData = () => {
    if (this.props.owner) {
      ...
      this.store.getTranslateChatList({
        order_id: this.props.router.query.status,
      });
    }
  };
  ...
  handleSendMessage = () => {
    ...
    if (!data.comment) {
      return message.error("ارسال متن خالی امکان پذیر نیست");
    }
    if (this.props.owner) {
            ...
            <div className="">
              {this.store.messageList.map((item, index) => {
                ...
                return (
                  <div
                    ...
                    className={`${
                      this.props.owner == 0
                        ? item.by_user === this.props.CoreStore.user.role
                          ? "messagebox-me"
                          : "messagebox"
                        : item.by_user === this.props.CoreStore.user.role
                        ? "messagebox"
                  ...
                  >
                    <div className="user-message">
                      <img
                        src={`/static/images/${
                          item.by_user === "translator" ? "order" : "man"
                        }.png`}
                        width={40}
                      />
                      <span>
                        {item.by_user === "translator" ? "مترجم" : "کاربر"}
                      </span>
                    </div>
                    ...
                    <div className="d-flex justify-content-between ">
                      <img
                        src={`/static/images${
                          item.seen ? "/double-check" : "/check1"
                        }.svg`}
                        width={14}
                      />
                      <span className={"sub-title-gray mx-2 "}>
                        {moment(item.created_at).locale("fa").fromNow()}
                      </span>
                    </div>
                  </div>
                );
              ...
              {this.store.hasMore && (
                <div className={"d-flex my-5 "}>
                  <Button
                    className={"sample-btn  mx-auto radius w-25"}
                    loading={this.store.loadingMore}
