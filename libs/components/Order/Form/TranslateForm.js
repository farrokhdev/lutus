...
import {
  ...
  TextIcon,
  UploadIcon,
} from "../../../../public/static/images/Images";
...
class TranslateForm extends Component {
    ...
    return (
      <>
        ...
        <span className={"text-muted  "}>
          تعداد کلمه: {this.state.text.split(/\b\S+\b/).length - 1}
        </span>
      </>
    );
  };
  ...
  _renderContentBox = () => {
    ...
    const validate = (value) => {
      if (validator.isURL(value)) {
        ...
        this.setState({
          link: value,
          error: "لینک مورد نظر خود را صحیح وارد کنید",
        });
      }
    };
    ...
    const deleteFile = (i) => {
      ...
      this.setState({
        fileList: this.state.fileList.filter((item) => item.uid !== i.uid),
      });
    };

    return (
      <div className={"col mx-auto"} id={"linkInput"}>
        <div
          className={
            "col mx-auto d-flex justify-content-around bg-gray radius p-2"
          }
        >
          <div
            onClick={() => changeType("link")}
            className={`box-upload ${
              content_type === "link" && "box-uploadActive"
            } `}
          >
            <LikIcon />
          ...
          <div
            ...
            className={`box-upload ${
              content_type === "file" && "box-uploadActive"
            }`}
          >
            <UploadIcon />
          ...
          <div
            ...
            className={`box-upload ${
              content_type === "text" && "box-uploadActive"
            }`}
          >
            <TextIcon />
        ...
        <div className="mt-4  content-type mb-3 ">
          {content_type === "link" && (
            <div className={"mt-5"}>
              <span className={"sub-title "}>
                لینکی که حاوی متن یا فایل مورد نظر برای ترجمه است را وارد کنید
              </span>
              <Input
                className={"link-input mt-3 "}
                placeholder={"مثال:‌  http://exmaple.com/file/translate "}
          ...
          {content_type === "file" ? (
            file ? (
              <>
                <span
                  className={"text-success my-2 d-flex justify-content-center"}
                >
                  فایل شما با موفقیت ارسال شد
                </span>
                <span
                  className={"text-muted my-2 d-flex justify-content-center"}
                >
                  اکنون میتوانید به مرحله بعد بروید، اگر میخواهید فایل دیگری را
                  جایگزین این فایل کنید روی دکمه ضربدر کلیک کنید
                </span>
                <div className="uploaded-item">
                  ...
                  <img
                    className={"cursor m-2 "}
                    onClick={() => deleteFile(this.state.fileList[0])}
              ...
              <Dragger
                ...
                type={
                  "jpeg, jpg, png, webp, gif, pdf, zip, doc, docx, rar, csv, ptt, pttx, xlsx, xls, ppt, pptx"
                }
                onRemove={(i) => deleteFile(i)}
                customRequest={(res) => this.handleUpload(res)}
              >
                <Spin
                  size="large"
                ...
                <div className={"d-flex flex-column"}>
                  <PlusOutlined />
                  <span className={"sub-title-gray my-2 "}>
                    فایل مورد نظر خود را آپلود کنید
                  </span>
                </div>
                {showUploadError && (
                  <p className={"text-danger mt-2"}>{uploadErrorMsg}</p>
                )}
              </Dragger>
            )
          ) : null}
  ...
  _renderTypeOfProject = () => {
    const { t } = this.props;
    return (
      <div className={"mb-4"} id={"projectType"}>
        {this.state.required && (
        ...
        <div
          ...
          className={`box-type ${
            this.state.typeProject === "system" && "box-typeActive"
          }`}
        >
          <div className="">
          ...
          <span className="text-muted">
            سفارش شما پس از ثبت توسط تیم پشتیبانی بررسی شده و سپس به مناسب ترین
            مترجم جهت ترجمه ارسال می شود.
          </span>
        </div>
        <div
          onClick={() => {
          ...
          className={`box-type ${
            this.state.typeProject === "freelancer" && "box-typeActive"
          }`}
        >
          <div className="">
          ...
          <span className="text-muted mb-3 ">
            سفارش شما به همه مترجمین فریلنسر جهت تعیین قیمت و زمان ارسال می شود
            و شما می توانید از بین آنها انتخاب نمایید.
          </span>
          <div className="d-flex flex-column ">
            <Checkbox.Group
              disabled={this.state.typeProject !== "freelancer"}
              value={this.props.store.handleOrder.freelancer_option}
              onChange={(e) =>
                this.props.store.changeValue("freelancer_option", e)
              }
            >
              <Checkbox className={"pl-1 ml-2 "} value="AllTranslator">
                {t("form.AllTranslator")}
              </Checkbox>
              <Checkbox className={"pl-1 ml-2 "} value="MeTranslator">
                {t("form.MeTranslator")}
              </Checkbox>
              <Checkbox className={"pl-1 ml-2 "} value="SiteTranslator">
                {t("form.SiteTranslator")}
              </Checkbox>
            </Checkbox.Group>
          </div>
        </div>
  ...
  _BackBtn = () => {
    ...
    this.props.store.handleOrder.current =
      this.props.store.handleOrder.current - 1;
  };
        ...
        <div className="projectView m-3 w-50 ">
          <div className="d-flex align-items-start">
            ...
            <div className="d-flex flex-column px-2">
              <div className="mt-2">
                <Link href={`/t/${profileView.info.username}`}>
                  <a>
                    <span className="titr-title">
                      {profileView.info.name + " " + profileView.info.family}
                    </span>
                  </a>
                </Link>
              </div>
  ...
  _renderFieldsItem = () => {
    ...
    const {
      t,
      store: {
        handleOrder: { content_type, confidential, language_to_id, file },
      },
    } = this.props;
      ...
      case "translate-verbal":
        Fields = [
              ...
              return (
                <>
                  <div className="d-flex align-items-center">
                    ...
                    <span className={"pink-title mx-2"}>
                      {" "}
                      ثبت سفارش {title}
                    </span>
                  </div>
          ...
          },
          {
            ...
            onChange: (id, item) =>
              this.handleSelectDestinationLanguage(id, item),
          },
          {
            key: "field_id",
            label: t("form.fields"),
              ...
              return (
                <div className={"pb-4 w-100 "}>
                  ...
                  {this.props.store.handleOrder.confidential && (
                    <div className={"pb-4 warning-label "}>
                      {t("help.confident")}
                    </div>
                  )}
                </div>
      ...
      case "translate-subtitle":
        Fields = [
          ...
          },
          {
            ...
            colSpan: 10,
            render: () => {
              return <div className={"mb-3"}>{this._renderContentBox()}</div>;
            },
          },
          {
            key: "space0",
            ...
            render: () => {
              return (
                <>
                  <br />
                  <hr />
                  {this.props.router.query.username ? (
                    <span className={"titr-title"}>مترجم منتخب</span>
                  ) : (
          ...
          },
          {
            ...
            colSpan: 10,
            render: () => {
              return this.props.router.query.username
                ? this._renderTranslatorInvite()
                : this._renderTypeOfProject();
            },
          },
          {
            key: "description",
        ...
        Fields = [
          ...
          },
          {
            ...
            render: () => {
              return (
                <Checkbox.Group
                  defaultValue={this.props.store.handleOrder.Additional_items}
                  // options={options2}
                  className={"row"}
                  onChange={(e) =>
                    (this.props.store.handleOrder.additional_items = e)
                  }
                >
                  {options2.map((option, index) => {
                    return (
                      <div className="col-4" key={index}>
                        <Checkbox className={"py-2"} value={option.value}>
                          {" "}
                          {option.label}
                        </Checkbox>
                      </div>
        ...
        switch (item.input_type) {
          ...
          case "number":
            Fields.splice(6, 0, {
              ...
              render: () => {
                return (
                  <div id={"face_translate"}>
                    <Radio
                      value={item.title}
                      onChange={(e) =>
                        this.props.store.handleOrder.setVal(
                          "verbal",
                          item.title
                        )
                      }
                ...
                return (
                  <>
                    ...
                    <Radio.Group
                      ...
                      options={options}
                      className={"py-2"}
                      disabled={!this.props.store.handleOrder[item.slug]}
                    />
                  </>
                );
              },
                ...
                return (
                  <div className={"d-flex "}>
                    <Radio.Group
                      ...
                      onChange={(e) =>
                        this.formRef.current.validateFields([item.slug])
                      }
                      className={"pt-2"}
                    >
                      {item.items.map((feature, index) => {
                        return (
                          <div className={"d-flex "} key={"b2" + index}>
                            <Radio
                              value={feature.slug}
                              onChange={(e) => {
                                this.props.store.handleOrder.setVal(
                            ...
                            {feature.options &&
                              this.props.store.handleOrder.multimedia ===
                                feature.slug && (
                                <Radio.Group
                                  value={this.props.store.handleOrder.options}
                                >
                  ...
                  validator: (rule, value, callback) => {
                    return new Promise((resolve, reject) => {
                      if (this.props.store.handleOrder[item.slug]) {
                        ...
                        reject(
                          new Error(`لطفا یکی از گزینه های زیر را انتخاب کنید`)
                        );
                      }
                    });
                  },
  ...
  render() {
    ...
    return (
      <div className={"py-4 boxShadow-form mt-3"}>
        <StateView
          state={this.props.store.stateViewOrigin}
          errorButtonAction={this.props.getData}
        >
          <Form
            ref={this.formRef}
            layout="vertical"
          ...
          >
            ...
            <div className="row form-btn mb-2">
              <div className="col-4">
                <Button onClick={() => this._BackBtn()} className={"backBTN"}>
                  قبلی
                </Button>
              </div>
              <div className="col-8">
  ...
  _sendForm = (e) => {
    ...
    this.props.router.query.username
      ? (this.props.store.handleOrder.project_type = "dedicated")
      : null;
    this.props.router.query.username
      ? (this.props.store.handleOrder.username = this.props.router.query.username)
      : null;

    if (this.props.store.handleOrder.delivery_date) {
      this.props.store.handleOrder.delivery_date = moment(
        this.props.store.handleOrder.delivery_date
      ).format("YYYY/DD/MM");
    }
