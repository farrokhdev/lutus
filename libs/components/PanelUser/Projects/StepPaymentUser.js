...
class StepPaymentUser extends Component {
  ...
  renderPaymentType = (status, file) => {
    switch (status) {
      ...
      case "pending":
        return (
          <Button disabled={true} className={" radius"}>
            در حال پردازش
          </Button>
        );
      case "paid":
      ...
      case "in_progress":
        return (
          <Button disabled={true} className={"btn-primary radius"}>
            در حال انجام
          </Button>
        );
      case "send_file":
        return (
          <Button
            disabled={!file.id}
            onClick={(e) => this.setState({ confirmVision: true })}
              ...
              <div className={"col-3"}>
                <div className="d-flex justify-content-center">
                  {item.file ? (
                    <Button
                      onClick={() =>
                        this.props.getFile(item.id, projectView.id)
                      }
                      icon={
                        <img
                          className={"mx-2"}
              ...
              <div className={"col-2"}>
                <div className="d-flex justify-content-center">
                  {item.file ? (
                    <Button
                      onClick={() =>
                        this.props.getFile(item.file, projectView.id)
                      }
                      icon={
                        <img
                          className={"mx-2"}
    ...
    return (
      <>
        <Card
          ...
          extra={
            this._renderEditButton() && (
              <Button
                loading={editLoading}
                className={"btn-warning radius mx-1"}
                onClick={this._renderEditPayment}
              >
                اصلاح قیمت و زمانبندی{" "}
              </Button>
            )
          }
        >
          {projectView.steps.length > 1
        ...
        <Modal
          ...
          title={"لطفا شیوه پرداخت را انتخاب کنید"}
          footer={[
            <Button onClick={(e) => this.setState({ visible: false })}>
              لغو
            </Button>,
            <Button
              disabled={!payment}
              loading={loading}
              onClick={this.sendPaymentForm}
              className={" addOrder-btn"}
            >
              پرداخت
            </Button>,
          ]}
        ...
        >
          <Radio.Group
            onChange={(e) => this.handlePayment(e.target.value)}
            className={"w-100"}
            value={payment}
          >
            <div className="row">
              {projectView.amount < wallet && (
                <div className="col-12">
                  <div className="" onClick={() => this.handlePayment(bank.id)}>
                    <div
                      className={`paymentMethod ${
                        bank.id === payment && "paymentMethod-Active"
                      } `}
                    >
                      <div className="d-flex align-items-center justify-content-between">
                        ...
                        <div className={"d-flex flex-column"}>
                          ...
                          <span className={"pink-title"}>
                            {this.formatNumber(wallet)} تومان{" "}
                          </span>
                        </div>
                      </div>
                    </div>
              ...
              <div className="col-12">
                <div className=" " onClick={() => this.handlePayment("bank")}>
                  <div
                    className={`paymentMethod ${
                      payment === "bank" && "paymentMethod-Active"
                    } `}
                  >
                    <div className="d-flex align-items-center">
                      <Radio value={"bank"} />
                        ...
                        <div className={"d-flex align-self-center"}>
                          ...
                          >
                            <g>
                              <path
                                xmlns="http://www.w3.org/2000/svg"
                                d="m511.386 281.608-38.714-219.559c-3.418-19.384-20.275-33.022-39.299-33.022-2.303 0-339.026 59.161-339.026 59.161-21.73 3.832-36.24 24.554-32.408 46.284l3.732 21.167h303.894c38.553 0 69.918 31.365 69.918 69.918v28.017 67.97 13.311l39.495-6.964c21.73-3.831 36.24-24.553 32.408-46.283z"
                              ></path>
                              <path
                                xmlns="http://www.w3.org/2000/svg"
                              ...
                              <path
                                xmlns="http://www.w3.org/2000/svg"
                                d="m0 253.574h409.518v67.97h-409.518z"
                              ></path>
                            </g>
                          </svg>
                        </div>
        ...
        <Modal
          visible={this.state.confirmVision}
          onCancel={() => this.setState({ confirmVision: false })}
          footer={null}
        >
          <Result
            status="success"
            ...
            extra={[
              <div className={"d-flex justify-content-center"}>
                <Button
                  ...
                  onClick={() => this._getConfirmProjectUser(projectView.id)}
                >
                  تایید
                </Button>
                <Button
                  className={"cancel-btn  mx-2 w-50"}
                  onClick={() => this.setState({ confirmVision: false })}
                >
                  لغو
                </Button>
              </div>,
