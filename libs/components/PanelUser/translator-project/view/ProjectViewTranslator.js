      ...
      <>
        <div className="">
          <StateView state={this.store.stateView} errorButtonAction={this._getData}>
            {projectView.resignation ? <Card title={"اعلام انصراف"} className="radius mb-3 ">

              <div className={"gray-title"}>انصراف شما ثبت شده</div>
              <div>انصراف شما ثبت شده و در صورت تائید ناظر سایت پروژه از لیست پرژه های شما حذف می شود.</div>
              ...
              <div className={'tex-right mt-2'}>
                <Popconfirm
                    ...
                    cancelText="لغو"
                >
                <Button loading={loadingEnseraf} type={'primary'}>صرف نظر از انصراف</Button>
                </Popconfirm>
              </div>
            </Card> : null}


            {projectView.status !== "pending" &&    <StepPaymentTranslator store={this.store} getData={this._getData} getFile={this.getFile} />}


            {projectView.status === "readiness" && <SelectProject store={this.store}/>}
            ...
            {projectView.status === "in_progress" && !projectView.resignation && <>
              ...
              <UploadBoxTranslator store={this.store} getData={this._getData} />
              <ProjectEndAction store={this.store}/>
            </>}


            {projectView.status === "awaiting_translator_accept" && <DedicateProject store={this.store}/>}


            <Card className="radius mb-3" title={"شماره سفارش:"+" "+ projectView.order_number } extra={ "روزمهلت تحویل:" +" " + projectView.details.deliver_date  }>
            ...
            {projectView.status === "awaiting_translator_accept_steps" && <TranslatorAcceptedStep store={this.store} getData={this._getData} />}





            {projectView.status === "freelancer_readiness" && (projectView.suggestion ?
          ...
          {projectView.status === "in_progress" && <ConversationBox owner={projectView.user_owner}/>}
