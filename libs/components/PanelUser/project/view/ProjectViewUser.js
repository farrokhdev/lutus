            ...
            <div className="d-flex align-items-center">
              ...
              <div className="d-flex flex-column px-2">
                <span className="titr-title">
                  {translator.name} {translator.family}
                </span>
      ...
      <div className={""}>
        ...
        >
          {projectView.status !== "pending" &&
            projectView.status !== "accept_detail" && (
              <StepPaymentUser
                store={this.store}
          ...
          {projectView.status === "accept_detail" && (
            <StepSelect store={this.store} getData={this._getData} />
          )}

          {(projectView.status === "confirm" ||
            projectView.status === "complete") && (
          ...
          {projectView.status === "protest" && (
            <Card className={"mb-3 radius"} title={"اعتراض شما ثبت شد"}>
              <span>
                اعتراض شما ثبت شده است و پس از بررسی توسط ناظر ،تعیین وضعیت
                پروژه به اطلاع شما خواهد رسید.
              </span>
            </Card>
          )}

          {projectView.status === "ended" && (
            <UserProjectEnded store={this.store} getData={this._getData} />
          )}
        </StateView>

        {translator && projectView.status !== "complete" ? (
