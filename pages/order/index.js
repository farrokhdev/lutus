...
class Order extends Component {
  ...
  _getData = () => {
    ...
    this.props.router.query.username &&
      this.store.getProfileView({ username: this.props.router.query.username });
  };
      ...
      <StateView state={this.store.stateView} errorButtonAction={this._getData}>
        <div className={"row py-4 "}>
          {this.store.ServiceList.map((item, index) => {
            return (
              <div key={"n" + index} className={"col-6 col-md-4 my-3 "}>
                <OrderCategoryBox
                  item={item}
      ...
      <>
        <div className="row text-center flex-column py-4">
          <h3 className={"text-success py-3 "}>
            تبریک! پروژه با موفقیت ایجاد شد
          </h3>
        ...
        <div className="row justify-content-around">
          <div className="d-flex align-items-center py-2 border-bottom">
            <div className="d-flex flex-column ">
              <span className="titr-title px-2 ">
                1- بررسی پروژه و قیمت گذاری
              </span>
              <span className="text-muted">
                پروژه شما در ارسع وقت توسط تیم پشتیبانی بررسی و قیمت گذاری شده
                اطلاع رسانی های لازم به شما صورت میگیرد
              </span>
            </div>
        ...
        <div className="row justify-content-center w-100">
          <Link href={`/panel/projectViewUser/${this.store.handleOrder.id}`}>
            <a>
  ...
  render() {
    let {
      handleOrder: { current, category, destination, origin },
    } = this.store;
            ...
            <div className="user-panel mt-3 mb-5 radius">
              <div className="row justify-content-center">
                ...
                <div className="col-12 col-md-6 py-3">
                  ...
                  >
                    {steps.map((item, indx) => (
                      <Step key={indx} title={item.title} />
                    ))}
                  </Steps>
                  {this._renderProgressItems()}
                  <div className="d-flex align-items-end justify-content-start steps-action"></div>
                </div>
              </div>
            </div>
          </div>
